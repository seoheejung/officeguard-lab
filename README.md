...
flowchart TD
    ...
    subgraph MainPC["Main PC - 보안 관측 서버"]
        ...
        Processing["PostgreSQL 저장 및 Rule 기반 분석"]
...
* Source IP와 Destination IP 기록
* Destination Port와 Protocol 기록
* 수집 가능한 경우 Domain과 Bytes In/Out 기록
* 수집 데이터를 `NETWORK_FLOW` 이벤트로 정규화
* 패킷 Payload와 HTTPS 본문 수집 제외
...
* 기존 Mini PC Agent에 Network Flow 수집 기능 추가
* Mini PC Network Flow 수집 방식 확정
* Mini PC의 실제 Network Flow Metadata 수집
