...
→ 이벤트 타입별 수신 데이터 검증
→ 검증된 이벤트를 기존 Kafka Producer로 전달
```
...
`FILE_COPIED`는 일반 파일 감시 경로의 파일을 USB 저장 장치로 복사한 경우 생성한다.

```text
AGENT_FILE_WATCH_PATH → USB 저장 장치
```
...
> Agent는 DNS 요청, Network Flow, 프로세스 실행, 파일 변경, USB 저장 장치 연결·해제, USB 대상 파일 복사 및 프린트 요청 이벤트를 수집한다.
>
> 수집한 이벤트는 공통 `SecurityEvent`로 변환해 Main PC Event Receiver로 전송한다.
>
...
### Phase 10. Mini PC Agent Endpoint Event 수집 ✅ 완료
